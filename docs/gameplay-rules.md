...
O núcleo inicial limita a espera final por conclusão a cinco segundos ativos adicionais e aborta com `evaluation-timeout` caso o coordenador avance até esse limite sem conclusão. Exceder buffers/interrupções usa `resource-limit`, preservando os registros já aceitos. Esses motivos são falhas de encerramento/recursos, não erros musicais. A etapa 06 acrescentou um julgador opcional à sessão: `advance` processa os prazos e conclui automaticamente quando todas as condições estão atendidas, antes de aplicar o timeout de espera por relatório. HOPO e caudas ainda não são aceitos por esse julgador.
