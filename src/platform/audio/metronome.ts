...
export interface AudioAnchor { readonly performanceMs: number; readonly audioSeconds: number }
...
export interface MetronomeRun {
  ...
  readonly continuation?: { readonly afterBeats: number; readonly delayMs: number; readonly beatOffset: number };
  onEnd(): void;
}
...
export class Metronome {
  ...
  constructor(private readonly onInterrupt: (reason: 'audio-suspended' | 'schedule-late' | 'context-changed') => void) {}
  get sampleRateHz(): number | null { return this.context?.sampleRate ?? null; }
  get outputId(): string | null {
    const sink = (this.context as (AudioContext & { sinkId?: unknown }) | null)?.sinkId;
  ...
  async enable(): Promise<boolean> {
    ...
    try {
      ...
      if (this.disposed || generation !== this.generation || context.state !== 'running') return false;
  ...
  start(run: MetronomeRun): void {
    ...
    if (run.continuation) {
      ...
      readInteger(run.continuation.beatOffset, 'metronome.continuation.beatOffset', 0, Number.MAX_SAFE_INTEGER);
    }
    readNumber(run.beats * 60_000 / run.bpm + (run.continuation?.delayMs ?? 0), 'metronome.duration', 1, 610_000);
  ...
  async dispose(): Promise<void> {
    ...
    if (context) {
      ...
      try { await context.close(); } catch { /* Recursos locais já foram desconectados. */ }
    }
  }

  private readonly stateChange = () => {
    if (!this.disposed && this.context?.state !== 'running' && !this.run?.silent) { this.stop(); this.onInterrupt('audio-suspended'); }
  };
  private readonly sinkChange = () => { this.stop(); this.onInterrupt('context-changed'); };
  ...
  private readonly tick = () => {
    ...
    if (!run.silent && this.context?.state !== 'running') { this.stop(); this.onInterrupt('audio-suspended'); return; }
    const timeAt = (beat: number) => run.startAtMs + beat * step
    ...
    while (this.nextBeat < run.beats && timeAt(this.nextBeat) <= now + 120) {
      ...
      if (wall < now + 5) { this.stop(); this.onInterrupt('schedule-late'); return; }
      if (!run.silent) {
        const beat = run.continuation && this.nextBeat >= run.continuation.afterBeats
          ? this.nextBeat - run.continuation.afterBeats + run.continuation.beatOffset
          : this.nextBeat + (run.beatOffset ?? 0);
        try { this.click(wall, beat % 4 === 0); }
        catch { this.stop(); this.onInterrupt('audio-suspended'); return; }
      }
      this.nextBeat++;
    }
    if (now >= timeAt(run.beats)) { this.stop(); run.onEnd(); }
  };
  ...
  private click(wall: number, accent: boolean): void {
    ...
    oscillator.start(when); oscillator.stop(when + 0.05);
  }
}
